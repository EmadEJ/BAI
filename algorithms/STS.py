...
class STS:
    def __init__(self, n, k, confidence, A, mode = {'use_optimized_p': False, 'average_w': False, 'average_points_played': False}):
        self.A = A
        self.n = n 
        self.k = k 
        self.T = 0 
        ...
        self.exploration_vector = find_projected_on_simplex_equivalent_in_X_space(A, 1/k * np.ones(k))
        ...
        self.exploration_vector_Z = (A.T @ self.exploration_vector) / np.sum(A.T @ self.exploration_vector)
    ...
    def best_empirical_arm_calculator(self):
        mu_hat = self.sum_of_rewards / self.K_times_seen

        actions_mu_hat = np.dot(self.A, mu_hat)
    ...
    def lambda_hat(self):
        ...
        result = 1 / (np.sum((self.A - self.A[best_arm])**2 / self.K_times_seen, axis=1)) 
    ...
    def Stopping_Rule(self):
        ...
        c_t = c_hat_sep(self.k, self.K_times_seen, self.confidence)
        return c_t >= lambda_hat_t


    def Alternate_Stopping_Rule(self):
        lambda_hat_t = self.lambda_hat() 
        c_t = c_hat_t(self.n, 1 , self.T, self.confidence)
        return c_t >= lambda_hat_t
    ...
    def C_Stopping_Rule(self):  
        ...
        V = - self.A + self.A[best_arm]        
        expected_K = self.A.T @ self.N_times_seen

        threshold = np.sqrt(
            2 * (
                self.k * Cg(np.log((self.n - 1) / self.confidence)/self.k) + 
                np.sum (2 * np.log(4 + np.log(self.K_times_seen)))
            ) * np.sum(V ** 2 / expected_K, axis = 1)
        )
        
        return SecondMin(delta_hat - threshold) < 0
    ...
    def Optimal_W(self):
        if not self.mode['use_optimized_p']:
            ...
            OPT_X = convert_back_to_X_space(self.A, OPT_Z)
    ...
    def C_optimal_W(self):
        ...
        return lambda_var.value / np.sum(lambda_var.value)
